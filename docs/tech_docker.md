...
2. From the folder containing the Dockerfile, build the Docker image.
    - `docker build -t jucc .`
    - docker will load your local repo into a `/jucc` directory in the image

## Usage

1. Run the Docker image: `docker run -itd --name build jucc`
2. Run CMake:
...
- by default, the docker image starts in the `/jucc/build` directory
